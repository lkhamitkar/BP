...
func TestTerraformAwsInfraEksWorkerNodesBase(t *testing.T) {
	t.Parallel()
	var err error
	...
	workerNodes1Path := test_structure.CopyTerraformFolderToTemp(t, "..", "/")
	...
	err = files.CopyFile(workernodestfvarsFile, filepath.Join(workerNodes1Path, workernodestfvarsFile))
	if err != nil {
		t.Fatal(err)
	}
	...
	test_structure.RunTestStage(t, "worker_nodes1", func() {
		workerNodes1Opts = terraform.WithDefaultRetryableErrors(t, &terraform.Options{
			...
			Vars: map[string]interface{}{
				...
				"node_group_tags": map[string]string{
					"Name": fmt.Sprintf("%s-%s", clusterId, nodegroupNamePrefix),
				},
			},
